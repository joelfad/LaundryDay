...
class Playing extends Component {
    ...
    render() {
        ...
        let dummyCards = [ {id: 1, text: "Sock"}, {id: 2, text: "Mitten"}, {id: 3, text: "Boot"}];
        let menuItems = dummyCards.map((card) => {
            return <MenuItem key={card.id} value={card.id}>{card.text}</MenuItem>;
        });
        ...
        let myCards = this.state.cards.map((item) => {
            return <MyCard key={item.id} card={item.text} />;
        });
